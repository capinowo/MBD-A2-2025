...
void Node<T>::borrowFromRightLeaf() {
    ...
    if (childIndex > 0) {
        nextParent->keys[childIndex - 1] = next->keys.front();
    }
}
...
Node<T>* Node<T>::splitInternal(int rIndex) {
    ...
    for (int i = 0; i < size - rIndex; i++) {
        newSiblingNode->children[i] = children[rIndex + 1 + i];
        newSiblingNode->children[i]->parent = newSiblingNode;
    }
   ...
   // Transfer keys ke sibling node
    for (int i = 0; i < newSiblingNode->size; i++) {
        newSiblingNode->keys[i] = keys[rIndex + 1 + i];
    }

    // Hapus keys yang sudah dipindah (termasuk key yang akan naik ke parent)
