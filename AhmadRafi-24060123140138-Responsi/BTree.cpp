...
void BTree<T>::remove(int key) { 
    ...
    if (index != -1) {
        ...
        if (curr && curr->size < (deg >> 1)) {
            
            int dummyKey = -999999; 
            remove(dummyKey, curr);
        }
    }
}
...
void BTree<T>::remove(int key, Node<T>* curr) { //
    ...
    if (curr->size < minCapacity) {
        if (curr->type == NodeType::NODE_ROOT) {
            ...
            Node<T>* parent = curr->parent;
            Node<T>* right = curr->next;  // Sebelumnya curr->right
    ...
    if (curr && curr->parent && curr->parent->size < minCapacity) {
        int dummyKey = -999999;
        remove(dummyKey, curr->parent);
    }
}
