...
void Node<T>::borrowFromRightLeaf() {
    ...
    size++;
    next->size--;
    int parentKeyIndex = nextParent->indexOfChild(next) - 1;
    nextParent->keys[parentKeyIndex] = next->keys.front();
}
...
void Node<T>::borrowFromLeftLeaf() {
    ...
    int parentKeyIndex = parent->indexOfChild(this) - 1;
    parent->keys[parentKeyIndex] = keys.front();
}
