...
void BTree<T>::remove(int key, Node<T> *curr) {
    ...
    if (curr->size < minCapacity) {
        if (curr->type == NodeType::NODE_ROOT) {
            ...
            // Jika root adalah leaf node dengan size < minCapacity, biarkan saja
            return; // Stop recursion for root node
            ...
            if (currChildIndex >= 0 && parent->children.size() > static_cast<size_t>(currChildIndex + 1)) {
            ...
            if (right && right->parent == parent && right->size > minCapacity) {
                ...
                curr->borrowFromRightInternal(right);
                ...
                curr->borrowFromLeftInternal(left);
                ...
                left->mergeWithRightInternal(curr);
                curr = left;
            }
        } else if (curr->type != NodeType::NODE_ROOT) { // Jika node adalah leaf, kita akan mencoba meminjam atau menggabungkan dengan sibling
            Node<T> *parent = curr->parent;
            if (!parent) return;
            
            int currChildIndex = parent->indexOfChild(curr);
            if (currChildIndex < 0) return;
            
            Node<T> *right = nullptr;
            Node<T> *left = nullptr;

            if (currChildIndex + 1 < parent->children.size()) {
                right = parent->children[currChildIndex + 1];
            }
    ...
    if (curr && curr->parent) {
        if (curr->parent->size < minCapacity) {
            remove(-1, curr->parent);
        }
    }
}
