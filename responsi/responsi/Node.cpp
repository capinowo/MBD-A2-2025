...
template <typename T>
Node<T>::Node() : parent(nullptr), prev(nullptr), next(nullptr), size(0), type(NodeType::NODE_LEAF){}
...
Node<T>* Node<T>::splitInternal(int rIndex) {
    ...
    newSiblingNode->keys.assign(keys.begin() + rIndex + 1, keys.end());
    newSiblingNode->children.assign(children.begin() + rIndex + 1, children.end());

    // Transfer children to new internal node
    for (Node<T>* child : newSiblingNode->children) {
        child->parent = newSiblingNode;
    }
