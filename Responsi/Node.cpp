...
void Node<T>::borrowFromRightLeaf() {
    ...
    nextParent->keys[nextParent->indexOfChild(this)] = next->keys.front();//Kesalahan 1
}
...
void Node<T>::mergeWithRightLeaf() {
    ...
    nextParent->keys.erase(nextParent->keys.begin() + childIndex); //Kesalahan 2
...
Node<T>* Node<T>::splitInternal(int rIndex) {
    ...
    for (int i = size - 1; i > rIndex; i--) {
        newSiblingNode->children[i - rIndex - 1] = children[i]; //kesalahan 3
