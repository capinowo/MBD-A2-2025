...
void Node<T>::borrowFromRightLeaf() {
    ...
    // Update separator key di parent
    if (parent) {
        int childIndex = parent->indexOfChild(this);
        if (childIndex >= 0 && static_cast<size_t>(childIndex) < parent->keys.size()) {
...
void Node<T>::mergeWithRightLeaf() {
    ...
    for (int i = 0; i < next->size; i++) {
        keys.push_back(next->keys[i]);
        vals.push_back(next->vals[i]);
    }
    size += next->size;
    ...
    // hapus key yang menghubungkan node ini dengan next
...
void Node<T>::mergeWithLeftLeaf() {
    ...
    for (size_t i = 0; i < keysToMerge.size(); i++) {
        prev->keys.push_back(keysToMerge[i]);
        prev->vals.push_back(valsToMerge[i]);
    }
    prev->size += keysToMerge.size();
